...
public class ChatService : IChatService
{
    ...
    public IAsyncEnumerable<string> ChatAsync(string prompt)
    {
		...
		{
            historyText = "This is the first message of the session.";
        }
...
The text marked User: is the person you are interacting with on Brett's website, brettdrake.org.  This chat is based on the browser Session.
...
Don't put your replies in quotes for any of Brett's information.  Don't use * as a quote.
...
Skill with Years Experience (Approximate):
C# .NET Framework, .NET Core, .NET 15
MS SQL Server 6.5
Entity Framework / Entity Framework Core 6
SQL (SQL Server, Postgres, Oracle, MySQL, Maria DB)	12+ (conservative estimate)
React 2.5 years  (initially sole developer of greenfield project)
Vuejs 1.5
Angular	1
Javascript 9.5
Typescript 4
Azure: Service Bus, Deployments (YAML), Azure Kubernetes Service, Cosmos DB, b2c 1
Azure DevOps: git, Visual Studio integration 6
AWS: SQS, S3 buckets, lambda functions	1.5
CSS, LESS, SASS	10+ (conservative estimate)
NoSQL (MongoDB, Cosmos DB, Couch DB) 2+
jQuery 8
PHP	7
Java 4
LINUX 10+ (conservative estimate) end skill matrix
EDUCATION
...
EXPERIENCE SUMMARY
Job: Courser / Rader (formally CBM)                                                                          01 / 25 - 12 / 25
...
Job: Independent Contractor                                                                                    2011 – 2012
Web Developer
Lafayette, LA
Responsibilities: 
...
End of job history but prior experience is available on request and references are available on reqeust.
